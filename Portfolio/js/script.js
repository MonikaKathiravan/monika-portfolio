...
document.addEventListener("DOMContentLoaded", () => {
  ...
  resumeLinks.forEach((link) => {
    link.addEventListener("click", async (e) => {
      ...
      // Get the filename from the download attribute or href
      const downloadName = link.getAttribute("download") || "download.pdf";
      ...
      try {
        ...
        // Force PDF mime type if it's the resume
...
// ==============================================================
// 21. RESUME — Force Download (Works from Hero + Contact)
// ==============================================================
(function () {
  "use strict";

  /**
   * Force-download a file using fetch + blob.
   * Bypasses the browser's built-in PDF preview.
   */
  async function forceDownload(fileUrl, fileName) {
    try {
      ...
      const tempLink = document.createElement("a");
      tempLink.href = blobUrl;
      tempLink.download = fileName || "resume.pdf";
      ...
      // Fallback: navigate directly (will open in new tab as last resort)
  ...
  /**
   * Wire up a resume button with visual feedback.
   */
  function wireResumeButton(btn) {
    if (!btn || btn.dataset.resumeWired === "true") return;
    btn.dataset.resumeWired = "true";

    btn.addEventListener("click", async (e) => {
      e.preventDefault();
      e.stopPropagation();

      const fileUrl = btn.dataset.resume;
      const fileName = btn.dataset.filename || "resume.pdf";

      if (!fileUrl) {
        console.warn("⚠️ Resume button missing data-resume attribute");
        return;
      }
      ...
      btn.innerHTML = `<span class="icon" style="display:inline-flex;"><i class="fas fa-spinner fa-spin"></i></span> Downloading…`;

      // Perform the download
      const success = await forceDownload(fileUrl, fileName);

      // Show success/fail state briefly
      if (success) {
        btn.innerHTML = `<span class="icon" style="display:inline-flex;color:#34d399;"><i class="fas fa-check"></i></span> Downloaded!`;
      } else {
        btn.innerHTML = `<span class="icon" style="display:inline-flex;color:#f87171;"><i class="fas fa-exclamation-triangle"></i></span> Try again`;
      }
      ...
      if (typeof window.confettiBurst === "function") {
        ...
        // Try to click on the confetti system via a synthetic event
        // (Your existing confetti system listens for clicks on elements containing "resume"/"download")
        const confettiEvent = new MouseEvent("click", { bubbles: true });
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // Wire all elements with data-resume attribute
    document.querySelectorAll("[data-resume]").forEach(wireResumeButton);

    // Also wire by specific IDs as a safety net
    wireResumeButton(document.getElementById("heroResumeBtn"));
    wireResumeButton(document.getElementById("contactResumeBtn"));
  });
})();

// ==============================================================
// 22. NAVBAR — Scroll, Active Link, Mobile Toggle
// ==============================================================
(function () {
  "use strict";
...
// ==============================================================
// 25. EXPERIENCE — Image Gallery Switcher
// ==============================================================
document.addEventListener("DOMContentLoaded", () => {
  const galleries = document.querySelectorAll(".exp-gallery");

  galleries.forEach((gallery) => {
    const main = gallery.querySelector(".exp-gallery-main");
    const images = gallery.querySelectorAll(".exp-gallery-img");
    const thumbs = gallery.querySelectorAll(".exp-thumb");
    const counter = gallery.querySelector(".exp-gallery-count");

    if (!main || images.length === 0) return;
    ...
    thumbs.forEach((thumb) => {
      thumb.addEventListener("click", (e) => {
        ...
        if (!isNaN(target)) switchImage(target);
      });
    });
    ...
    main.addEventListener("keydown", (e) => {
      const current = Array.from(images).findIndex((img) =>
        img.classList.contains("active"),
      );
      if (e.key === "ArrowRight") switchImage((current + 1) % images.length);
      if (e.key === "ArrowLeft")
    ...
    main.addEventListener(
      ...
      (e) => {
        ...
        if (diff < 0) switchImage((current + 1) % images.length);
        else switchImage((current - 1 + images.length) % images.length);
      },
      { passive: true },
    );

    /* ---------- Zoom button → open lightbox ---------- */
    const zoomBtn = gallery.querySelector(".exp-gallery-zoom");
    if (zoomBtn && typeof openLightbox === "function") {
      zoomBtn.addEventListener("click", () => {
        // Collect all valid images in this gallery
        const validSrcs = [];
        const validTitles = [];
        const expCard = gallery.closest(".exp-showcase");
        const role =
          expCard?.querySelector(".exp-title h3")?.textContent.trim() || "";
        const company =
          expCard?.querySelector(".exp-company")?.textContent.trim() || "";

        images.forEach((img, i) => {
          if (img.naturalWidth > 0) {
            validSrcs.push(img.src);
            validTitles.push(
              `${role} — Image ${i + 1}${company ? " · " + company : ""}`,
            );
          }
        });
        ...
        const activeIdx = Array.from(images).findIndex((img) =>
          img.classList.contains("active"),
        );
        const mappedIdx = validSrcs.indexOf(images[activeIdx]?.src);

        openLightbox(validSrcs, validTitles, mappedIdx >= 0 ? mappedIdx : 0);
      });
    }

    /* ---------- If main image fails, activate fallback view ---------- */
    images.forEach((img) => {
      img.addEventListener("error", () => {
        // If this was the active one, try the next
        if (img.classList.contains("active")) {
          const next = Array.from(images).find(
            (i) => i !== img && i.style.display !== "none",
          );
          if (next) {
    ...
    console.log(
      "🎠 Experience gallery initialized with",
      images.length,
      "images",
    );
  });
...
// ==============================================================
// 26. RESEARCH — Scroll Reveal + Certificate Lightbox
// ==============================================================
document.addEventListener("DOMContentLoaded", () => {
  /* ==========================================================
       1. SCROLL REVEAL
  ...
  if (researchCards.length > 0) {
    ...
    researchCards.forEach((card) => revealObserver.observe(card));
  }
  ...
  /* ==========================================================
       ...
       ========================================================== */
  /**
   * Open a research certificate in the global lightbox.
   * Each certificate element has data-cert-src and data-cert-key.
   */
  function openResearchCert(element) {
    const src = element.dataset.certSrc;
    const key = element.dataset.certKey || "";
    if (!src) return;

    // Build a nice title based on the key
    const titleMap = {
      "cervical-acceptance":
        "Acceptance Certificate — Cervical Cancer Research",
      "cervical-presentation":
        "Presentation Certificate — Cervical Cancer Research",
      "6g-acceptance": "Acceptance Certificate — 6G Anomaly Detection",
      "6g-presentation": "Presentation Certificate — 6G Anomaly Detection",
    };
    const title = titleMap[key] || "Research Certificate";

    // Use the global lightbox if available
    if (typeof openLightbox === "function") {
      openLightbox([src], [title], 0);
    } else {
  ...
  /* ---------- Wire up all certificate elements ---------- */
  document.querySelectorAll(".research-cert").forEach((cert) => {
    cert.setAttribute("tabindex", "0");
    cert.setAttribute("role", "button");
    cert.setAttribute(
      "aria-label",
      cert.dataset.certKey
        ? `View ${cert.dataset.certKey} certificate`
        : "View certificate",
    );

    cert.addEventListener("click", () => openResearchCert(cert));

    // Keyboard access
    cert.addEventListener("keydown", (e) => {
      if (e.key === "Enter" || e.key === " ") {
        e.preventDefault();
        openResearchCert(cert);
      }
    });
  });

  console.log("🔬 Research section initialized with certificate lightboxes");
});
// ==============================================================
// FINAL LOGS
// ==============================================================
